<script lang="ts">
  ...
  function setPhysicianType(type: "same" | "different") {
    state.repeatPhysicianType =
      state.repeatPhysicianType === type ? null : type;
  }
</script>
...
<div class="py-2 pl-5">
  <label class="flex items-center gap-1.5">
    ...
    <span
      ...
      style:color="var(--color-text-primary)">Repeat Procedure</span
    >
  </label>
  ...
  {#if state.isRepeatProcedure}
    <div class="mt-2 pl-[26px]">
      ...
      <div class="mt-1 flex gap-1">
        {@render toggleBtn(
          "Same Physician",
          state.repeatPhysicianType === "same",
          () => setPhysicianType("same"),
        )}
        {@render toggleBtn(
          "Different Physician",
          state.repeatPhysicianType === "different",
          () => setPhysicianType("different"),
        )}
      </div>

      <p
        class="mt-2 font-epic text-[13px] font-bold"
        style:color="var(--color-text-primary)"
      >
        Reason:
      </p>
