<script lang="ts">
  ...
  if (applyPreset) {
    const presetMap: Record<string, () => void> = {
      normalEFAST: () =>
        config.presets.find((p) => p.label === "Normal E-FAST")?.apply(state),
      normalFAST: () =>
        config.presets.find((p) => p.label === "Normal FAST")?.apply(state),
      normalDvtBilateral: () =>
        config.presets
          .find((p) => p.label === "Normal Bilateral")
          ?.apply(state),
      normalDvtLeft: () =>
        config.presets.find((p) => p.label === "Normal Left")?.apply(state),
